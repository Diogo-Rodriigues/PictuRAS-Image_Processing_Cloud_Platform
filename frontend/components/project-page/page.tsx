...
import { useGetProjectResults, useGetSocket } from "@/lib/queries/projects";
...
export default function SharedProjectPage({
  ...
  // Only fetch results if we have the project data (need owner's user_id)
  ...
  }, [processingSteps, socket.data, totalProcessingSteps, sidebar, isMobile, projectResults, router]);
                    ...
                    <Button
                      ...
                      onClick={() => {
                        processProject.mutate(
                          { uid: project.data.user_id, pid: project.data._id, token: session.token },
                          {
                            onSuccess: () => {
                              setProcessing(true);
      ...
      <Transition show={processing}>
        <div className="absolute top-0 left-0 h-screen w-screen bg-black/70 z-50 flex justify-center items-center">
          <Card className="p-4 flex flex-col justify-center items-center gap-4">
            <div className="flex gap-2 items-center text-lg font-semibold">Processing <LoaderCircle className="animate-spin" /></div>
